...
function renderProfile(profileData) {
    ...
    bioElement.textContent = profileData.bio || "This user has no bio.";
    ...
    blogElement.href = profileData.blog ? (profileData.blog.startsWith('http') ? profileData.blog : `//${profileData.blog}`) : '#';
    blogElement.textContent = profileData.blog || '';

    twitterContainer.classList.toggle('hidden', !profileData.twitter_username);
    twitterElement.href = profileData.twitter_username ? `https://twitter.com/${profileData.twitter_username}` : '#'; // Corrected URL format
    twitterElement.textContent = profileData.twitter_username ? `@${profileData.twitter_username}` : '';
}

function renderRepos(repoData) {
    reposContainer.innerHTML = ''; s
